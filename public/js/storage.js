export const BUILDINGS = {
  ...
  armory: { name: '무기고', cost: 180, unique: true, desc: '산탄·탄환 계열 강화를 해금합니다.' },
  lab: { name: '연구소', cost: 220, unique: true, desc: '드론과 선택지 재배치를 해금합니다.' },
  ...
  comms: { name: '통신탑', cost: 250, unique: true, desc: '고철 회수율과 외부 기록을 해금합니다.' }
};
...
export function productionRate(meta){
  ...
  return lvl ? (1 + (lvl-1)) : 0; // energy per minute
}
...
export function getSynergies(meta){
  ...
  return [
    {id:'power-armory', active:hasAdj('power','armory'), name:'과급 탄약', desc:'공격속도 +10%', perk:{fireRate:.10}},
    {id:'power-lab', active:hasAdj('power','lab'), name:'고효율 연구', desc:'레벨업 재배치 +1회', perk:{reroll:1}},
    {id:'green-hq', active:hasAdj('greenhouse','hq'), name:'생체 보급', desc:'최대 체력 +15', perk:{maxHp:15}},
    {id:'work-armory', active:hasAdj('workshop','armory'), name:'현장 개조', desc:'공격력 +8%', perk:{damage:.08}},
    {id:'comms-hq', active:hasAdj('comms','hq'), name:'회수 지령', desc:'고철 보상 +5%', perk:{scrap:.05}}
  ];
}

export function getRunPerks(meta){
  const perks={ damage:0, fireRate:0, maxHp:0, scrap:0, reroll:0, unlockDrone:false, unlockSpread:false };
  perks.damage += (meta.training?.damage || 0) * .02;
  perks.maxHp += (meta.training?.hp || 0) * 3;
  perks.scrap += (meta.training?.salvage || 0) * .015;
  ...
  const comms=buildingLevel(meta,'comms'); if(comms) perks.scrap += .02*comms;
  const lab=buildingLevel(meta,'lab'); perks.unlockDrone=!!lab;
  const armory=buildingLevel(meta,'armory'); perks.unlockSpread=!!armory;
  for(const s of getSynergies(meta).filter(s=>s.active)) for(const [k,v] of Object.entries(s.perk)) perks[k]=(perks[k]||0)+v;
  return perks;
}
...
export const STORIES = {
  ...
  lab: { title:'기록 03 — 연구동', text:'연구소 데이터의 대부분은 지워졌지만, 자동 전투 보조체의 시험 기록이 남아 있다. “대상 스스로 전술을 바꾸기 시작함.”' },
  ...
  comms: { title:'기록 06 — 첫 신호', text:'통신탑 복구 직후 짧은 신호가 잡혔다. “전초기지 01, 응답하라. 북쪽은 아직 살아 있다.”' }
};
